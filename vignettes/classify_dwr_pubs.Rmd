...
classify_pubs_gemini_code <- deparse(classify_pubs_gemini)
full_text <- paste(classify_pubs_gemini_code, collapse = "\n")
system_prompt_pattern <- "system_prompt\\s*<-\\s*paste\\(([^)]+)\\)"
system_prompt_match <- regexpr(system_prompt_pattern, full_text, perl = TRUE)

if (system_prompt_match > 0) {
  captured_text <- regmatches(full_text, system_prompt_match)
  
  system_prompt_content <- gsub(
    "system_prompt\\s*<-\\s*paste\\((.*)\\)", "\\1",
    captured_text
  )
  
  system_prompt_lines <- strsplit(system_prompt_content, ",\\s*\"|\",\\s*")[[1]]
  system_prompt_lines <- gsub("^\\s*\"|\"\\s*$", "", system_prompt_lines)
  system_prompt_lines <- gsub("^\\s+|\\s+$", "", system_prompt_lines)
  system_prompt_lines <- system_prompt_lines[system_prompt_lines != ""]
  
  for (line in system_prompt_lines) {
    cat("> *", line, "*\n>\n", sep="")
  }
} else {
  cat("> *Could not extract system prompt from function.*")
}
```
...
LLM produced a three-level National Science Foundation (NSF) classification for 
each article.
